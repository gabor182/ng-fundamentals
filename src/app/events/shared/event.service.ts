...
export class EventService {
	...
	searchSessions(term: string) {
		var searchTerm = term.toLocaleLowerCase();
		var results: ISession[] = [];
		EVENTS.forEach(event => {
			var matches = event.sessions.filter(sess => sess.name.toLocaleLowerCase().indexOf(searchTerm) > -1);
		...
		// http async method simulation with EventEmitter
